...
public class Week06TicTacToeLab {
	...
	//will show these numbers so the players know what the space is called without thinking. and it looks nicer. to me.
	//change each element to X or O as we go. 
		
	
	String playerTurn = "X"; //Starting with X
	Boolean winStatus = false;
	int turns = 1;
	
	gameboard.display(boardSquares);
	while ((turns < 10) && (winStatus == false)) {
		System.out.println(playerTurn + "'s Turn!");

		gameboard.oneTurn(playerTurn, boardSquares); 
		//asking for square, checking if square is available, and if so, changing space to X or O
	 	
	 	gameboard.display(boardSquares);
	 
	 	winStatus = gameboard.checkWinStatus(boardSquares, winStatus);
	 	if (winStatus == true) {
	...
	//METHODS
//	public static void endGame(String playerTurn) {
//		System.out.println(playerTurn + " Wins!");
//	}
